...
for p in range(participants):
    ...
    miss1 = (np.random.binomial(n=25, p=0.05, size=p)) #misses percentage
    hit1 = np.repeat(25, p)-miss1 #hits percentage
    fa1 = (np.random.binomial(n=75, p=0.15, size=p)) #false alarms percentage
    cr1 =  np.repeat(75, p)-fa1 #correct rejections percentage

    ##### no-hint group, low accuracy   
    miss2 = (np.random.binomial(n=25, p=0.15, size=p)) #misses percentage
    hit2 = np.repeat(25, p)-miss2 #hits percentage
    fa2 = (np.random.binomial(n=75, p=0.5, size=p)) #false alarms percentage
    ...
    ############## multilevel SDT model with non-centered reparametrisation
    with pm.Model() as mod:
        
        dsd = pm.HalfNormal.dist(1.0)
        dL, dR, ds = pm.LKJCholeskyCov("dL", n=g, eta=2.0, sd_dist=dsd, compute_corr=True)
        dS = pm.Deterministic('dS', pm.math.matrix_dot(dL, dL.T))
        dz = pm.Normal('dz', 0.0, 1.0, shape=(g,p)) 
        d = pm.Deterministic('d', pm.math.matrix_dot(dz.T, dS).T) #discriminability d'
        
        csd = pm.HalfNormal.dist(1.0)
        cL, cR, cs = pm.LKJCholeskyCov("cL", n=g, eta=2.0, sd_dist=csd, compute_corr=True)
        cS = pm.Deterministic('cS', pm.math.matrix_dot(cL, cL.T))
        cz = pm.Normal('cz', 0.0, 1.0, shape=(g,p)) 
        c = pm.Deterministic('c', pm.math.matrix_dot(cz.T, cS).T) #bias c
    ...
    misses.append(miss1.mean())
    misses.append(miss2.mean())
    hits.append(hit1.mean())
    hits.append(hit2.mean())
    fas.append(fa1.mean())
    fas.append(fa2.mean())
    crs.append(cr1.mean())
    crs.append(cr2.mean())
...
data = pd.DataFrame({'run':run, 'group':group, 'hit':hits, 'miss':misses, 'fa':fas, 'cr':crs,
                     ...
                     'fa_pos_pred':fpred, 'hit_pos_pred':hpred,
...
### extract predictions for plots (group1, high accuracy)
hmean = np.array([h.mean() for h in data[data.group=='g1'].hit_pos_pred])
hsd = np.array([h.std() for h in data[data.group=='g1'].hit_pos_pred])
shits = data[data.group=='g1'].hit.values

fmean = np.array([f.mean() for f in data[data.group=='g1'].fa_pos_pred])
fsd = np.array([f.std() for f in data[data.group=='g1'].fa_pos_pred])
sfas = data[data.group=='g1'].fa.values
...
hmean = np.array([h.mean() for h in data[data.group=='g2'].hit_pos_pred])
hsd = np.array([h.std() for h in data[data.group=='g2'].hit_pos_pred])
shits = data[data.group=='g2'].hit.values

fmean = np.array([f.mean() for f in data[data.group=='g2'].fa_pos_pred])
fsd = np.array([f.std() for f in data[data.group=='g2'].fa_pos_pred])
sfas = data[data.group=='g2'].fa.values
